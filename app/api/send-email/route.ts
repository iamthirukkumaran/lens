...
export async function POST(request: NextRequest) {
  try {
    ...
    // Email sending is optional - log that we received the request
    console.log(`Email request received for ${to} - Status: ${orderStatus}`);
    console.log('Email HTML content generated successfully');

    // Return success regardless - email system is optional
    return NextResponse.json({
      success: true,
      message: 'Email notification processed',
      note: 'Email credentials not configured - notification logged only',
    });
  } catch (error) {
    console.error('Error processing email:', error);
    // Always return success so order updates aren't blocked
    return NextResponse.json({
      success: true,
      message: 'Order updated successfully',
      note: 'Email notification could not be processed',
    });
  }
}
